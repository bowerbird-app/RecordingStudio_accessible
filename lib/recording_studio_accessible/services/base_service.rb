  ...
  module Services
    ...
    class BaseService
      ...
      def run_before_hooks
        ...
        RecordingStudioAccessible::Hooks.run(:before_service, self.class, service_args)
      end
      ...
      def run_after_hooks(result)
        ...
        RecordingStudioAccessible::Hooks.run(:after_service, self.class, result)
      end
      ...
      def run_with_around_hooks
        if hooks_enabled? && RecordingStudioAccessible.configuration.hooks.registered?(:around_service)
          RecordingStudioAccessible::Hooks.run_around(:around_service, self) { perform }
        else
          perform
        end
      end

      # Check if hooks are enabled (RecordingStudioAccessible is loaded and configured)
      #
      # @return [Boolean]
      def hooks_enabled?
        defined?(RecordingStudioAccessible) &&
          RecordingStudioAccessible.respond_to?(:configuration) &&
          RecordingStudioAccessible.configuration.respond_to?(:hooks)
      end
