...
class RecordableDeclarationsTest < ActiveSupport::TestCase
  test "dummy app declarations are valid for RecordingStudio 4" do
  ...
  test "dummy app recordables declare root and parent rules" do
    ...
    refute RecordingStudio.shared_root_type?(Workspace)
  ...
  test "access declaration is child-only and follows accessible child opt ins" do
    ...
    assert_equal [ "Folder", "MessageGroup", "Workspace" ], RecordingStudio.capability_allowed_parent_types_for(RecordingStudio::Access)
    assert_equal [ "Folder", "MessageGroup", "Workspace" ], RecordingStudio.allowed_parent_types_for(RecordingStudio::Access)
