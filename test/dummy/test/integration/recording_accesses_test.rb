...
class RecordingAccessesTest < ActionDispatch::IntegrationTest
  ...
  test "admin can view the recording access page with the blank engine layout" do
    ...
    assert_includes @response.body, %(<form class="button_to inline" method="post" action="#{recording_accesses_path}/#{direct_access_recording_for(@editor).id}")
    assert_includes @response.body, %(name="_method" value="delete")
    refute_includes @response.body, "Main navigation"
    refute_includes @response.body, "Sign out"
  end

  test "root recording hides inherited access section when there are no ancestors" do
    sign_in @admin
  ...
  test "admin can view the new access page" do
    ...
    get "#{recording_accesses_path}/new"

    assert_response :success
    assert_includes @response.body, "New access"
    assert_includes @response.body, "Add someone new"
  ...
  test "admin can view the edit access page" do
    ...
    get "#{recording_accesses_path}/#{access_id}/edit"
  ...
  test "admin can add, edit, and remove direct access" do
    ...
    patch "#{recording_accesses_path}/#{access_recording.id}", params: {
      access: {
        role: "edit"
      }
    }
    ...
    delete "#{recording_accesses_path}/#{access_id}"
  ...
  test "admin sees an error when the email does not resolve to a user" do
    sign_in @admin
    ...
    assert_includes @response.body, "Actor is required"
  ...
  def direct_access_recordings_for(user)
    RecordingStudio::Services::AccessCheck.access_recordings_for(@folder_recording)
