...
class HooksTest < Minitest::Test
  def setup
    @hooks = RecordingStudioAccessible::Hooks.new
  end
  ...
  def test_raise_on_error_raises_hook_error
    ...
    assert_raises(RecordingStudioAccessible::Hooks::HookError) do
      @hooks.run(:after_initialize)
    end
  end
  ...
  def test_class_run_delegates_to_configuration
    RecordingStudioAccessible.configuration.hooks
    called = false

    RecordingStudioAccessible.configuration.hooks.after_initialize { called = true }
  ...
  def test_class_trigger_is_alias_for_run
    called = false
    RecordingStudioAccessible.configuration.hooks.on(:custom_event) { called = true }

    RecordingStudioAccessible::Hooks.trigger(:custom_event)
